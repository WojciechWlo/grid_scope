...
def getSpreadsheetsOut(request):
    spreadsheetsOut = SpreadsheetOut.objects.all().order_by('-id')
...
def createSpreadsheetOut(request):
    ...
    if not label or not spreadsheet_label or not data_cell or worksheet_name==None:
        response = Response(
...
def editSpreadsheetOut(request, pk):
    ...
    spreadsheet_out.worksheet_name = worksheet_name
